...
@ActivityFragmentInject(contentViewId = R.layout.activity_register_phone_confirmation, withLoadingAnim = true)
public class RegisterPhoneConfirmationActivity extends BaseActivity<IRegisterPresenter> implements IRegisterView {
        ...
        if (data != null) {
            if (data instanceof UserLoginResponse) {
                ...
                new VexDialog.Builder(this)
                        ...
                        .content(getString(R.string.register_confirmation_sms_success))
            ...
            new VexDialog.Builder(this)
                    .title(getString(R.string.succsess))
                    .content(getString(R.string.register_confirmation_resend_sms_success))
                    .optionType(DialogOptionType.OK)
                    .onPositive(new VexDialog.MaterialDialogButtonCallback() {
                        @Override
                        public void onClick(@NonNull VexDialog dialog, @NonNull DialogAction which) {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.resend_button:
                if (isAbleToResend()) {
                    mPresenter.requestResendSMS(user.getId());
                }
                break;
            case R.id.confirmation_button:
                if (user != null) {
                    ...
                    if (!TextUtils.isEmpty(code)) {
                        mPresenter.requestSmsConfirmation(user.getId(), code);
    ...
    public boolean isAbleToResend() {
        ...
        return ((System.currentTimeMillis() - lastEmailSendTime) > StaticGroup.getSmsResendCountdown());
    }
            ...
            timeSubsription = Observable.interval(0, 1, TimeUnit.SECONDS)
                    ...
                    .subscribe(new Action1<Object>() {
                        ...
                        public void call(Object o) {
                            ...
                            if (!StaticGroup.isScreenOn(RegisterPhoneConfirmationActivity.this, true)) {
                                if (timeSubsription != null && !timeSubsription.isUnsubscribed()) {
                                    timeSubsription.unsubscribe();
                                }
                            } else {
    ...
    private void setWatchText() {
        ...
        if (getLastEmailSendTime() != 0) {
            ...
            next.setTimeInMillis(lastEmailSendTime + StaticGroup.getSmsResendCountdown());

            long remainTime = next.getTimeInMillis() - now.getTimeInMillis();

            if (remainTime < 0) {
                mTvCountdownVp.setText(getString(R.string.register_confirmation_resend_sms));
                ...
                String sBtn = String.format(getString(R.string.register_confirmation_resend_sms_time), time);
